    ...
    foreach($getable as $key) {
        if ($getCmd = ($_GET[$key] ?? null) AND is_string($getCmd)) {
            if (in_array($getCmd, $usedOptions))
                $exec[] = $getCmd;
            else $exec[] = sprintf('%s=%s', $key, 'class' === $key ? '"' . $getCmd . '"' : $getCmd);
        }
    }
    $out = json_decode($execRun = shell_exec(implode(' ', $exec)), true);
    if (!$out AND $execRun)
        $out['print'] = "unexpected content\n\n" . print_r($execRun, true);
...
<header>
    <form action="" method="get">
        ...
        <?php if (!($out['file'] ?? null) AND ($_GET['class'] ?? null)) { ?>
            <div>
                <span>class</span>
                <input type="text" name="class" value="<?= h($_GET['class']) ?>" />
            </div>
        <?php } ?>
...
<main>
    <pre><?= $printUseList ? trim(h($printUseList)) : "\n/**\n * Nothing to use\n */" ?></pre>
